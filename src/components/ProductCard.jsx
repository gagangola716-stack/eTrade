  ...
  const renderStars = () => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < Math.floor(rating || 5)) {
        ...
        stars.push(
          <i key={i} className="fa-regular fa-star text-secondary"></i>,
        );
      }
    }
    return stars;
  ...
  return (
    <>
      <div className="card border-0 product-card h-100 position-relative">
        <div
          className="position-relative bg-light rounded d-flex align-items-center justify-content-center p-4"
          style={{ height: "250px" }}
        >
          {/* Badges */}
          {discount && (
          ...
          <div className="position-absolute top-0 end-0 m-3 d-flex flex-column gap-2 z-2">
            ...
            >
              <i
                className={`${isLiked ? "fa-solid text-danger" : "fa-regular text-dark"} fa-heart`}
              ></i>
            </button>
            <button
              className="btn btn-sm btn-white rounded-circle shadow-sm"
          ...
          {/* Add to Cart Overlay */}
          <button
            className="btn btn-dark w-100 position-absolute bottom-0 start-0 end-0 rounded-bottom add-to-cart-btn z-2"
